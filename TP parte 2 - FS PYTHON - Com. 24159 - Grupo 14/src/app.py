...
def home():
    ...
    cursor.execute("SELECT * FROM form_contacto ORDER BY Fecha")
...
def agregarMensaje():
    ...
    if motivo and nombre and apellido and email and whatsapp and mensaje and adjunto and tc:
        cursor = db.database.cursor()
        sql = "INSERT INTO form_contacto (Motivo, Nombre, Apellido, E_mail, Whatsapp, Mensaje, Adjunto, TC) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)"
        data = (motivo, nombre, apellido, email, whatsapp, mensaje, adjunto, tc)
